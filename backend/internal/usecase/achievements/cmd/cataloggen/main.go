...
func main() {
	...
	output.WriteString("| # | Код | Название | Условие | Метрика | Порог | XP | Секретная |\n")
	output.WriteString("| ---: | --- | --- | --- | --- | ---: | ---: | :---: |\n")
	for _, definition := range achievements.Definitions() {
		...
		fmt.Fprintf(&output, "| %d | `%s` | %s | %s | `%s` | %d | %d | %s |\n",
			definition.SortOrder, definition.Code, definition.Title, definition.Description,
			definition.Metric, definition.Target, definition.XP, secret)
	}
